...
import { AirlinkCloudClient } from '../../handlers/utils/core/cynexCloud';
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        ...
        try {
          ...
          files = files.filter((file: any) => file.name !== 'cynex');
      ...
      async (req: Request, res: Response) => {
        ...
        try {
          ...
          if (response.data.success) {
            ...
            if (isCloudBackupEnabled) {
              try {
                ...
                if (uploadResult && uploadResult.id) {
                  ...
                  filePath = 'cynex-cloud'; // Marker for cloud backups
                }
              } catch (cloudError) {
                logger.error('Failed to redirect backup to Cynex Cloud:', cloudError);
            ...
            res.json({
              success: true,
              message: isCloudBackupEnabled && airlinkCloudId ? 'Backup created and uploaded to Cynex Cloud' : 'Backup created successfully',
              backup: {
                ...backup,
        ...
        try {
          ...
          if (backup.airlinkCloudId) {
            ...
            if (!settings?.airlinkCloudApiKey) {
              res.status(500).json({ error: 'Cynex Cloud API key not configured' });
              return;
            }
            ...
            try {
              ...
              logger.error('Failed to prepare Cynex Cloud backup for restore:', err);
          ...
          if (backup.airlinkCloudId && backupPath !== 'cynex-cloud') {
            axios.delete(
              `${daemonSchemeSync()}://${server.node.address}:${server.node.port}/container/backup`,
              {
      ...
      async (req: Request, res: Response) => {
        ...
        try {
          ...
          if (backup.airlinkCloudId) {
            ...
            if (settings?.airlinkCloudApiKey) {
              const cloudClient = new AirlinkCloudClient(settings.airlinkCloudApiKey);
              await cloudClient.deleteFile(backup.airlinkCloudId).catch(e => logger.warn(`Failed to delete backup from Cynex Cloud: ${e}`));
            }
          } else {
            try {
              await axios.delete(
