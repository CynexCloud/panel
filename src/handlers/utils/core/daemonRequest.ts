...
export function installDaemonRequestInterceptor(): void {
  axios.interceptors.request.use((config: InternalAxiosRequestConfig) => {
    ...
    config.headers.set('X-Cynex-Timestamp', String(timestamp));
    config.headers.set('X-Cynex-Signature', signature);
    config.headers.set('X-Cynex-Nonce', nonce);
    config.headers.set('X-Cynex-Payload-Version', String(HMAC_PAYLOAD_VERSION));
